...
// METHOD AND CONTINUING WITH HAMMING'S METHOD
...
int main(){
	...
	double yp[n+1],  			// First predictor
	ym[n+1], 					// Second predictor
	y[n+1];  					// Corrector
	y[0] = y0;
	...
	// ADAMS METHOD:
	for (int i=3 ; i<=n-1 ; i++){
		yp[i+1] = y[i-3] + 4.0*h*(2.0*f(x[i],y[i]) -f(x[i-1],y[i-1]) + 2.0*f(x[i-2],y[i-2]) )/3.0;  // First predictor
		ym[i+1] = yp[i+1] -112.0*( yp[i] - y[i] )/121.0;  // Second predictor
		y[i+1] = (9.0*y[i] -y[i-2] + 3.0*h*(f(x[i+1],ym[i+1]) + 2*f(x[i],y[i]) - f(x[i-1],y[i-1])))/8.0;  // Corrector
	}
	
	cout << "HAMMING'S METHOD:\n";
