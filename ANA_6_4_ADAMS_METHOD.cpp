...
// METHOD AND CONTINUING WITH ADAMS METHOD
...
int main(){
	...
	double y[n+1];				// Vector with solution y with global error O(h^3)
	y[0] = y0;
	...
	// ADAMS METHOD:
	for (int i=2 ; i<=n-1 ; i++){
		y[i+1] = y[i] + h*(23.0*f(x[i],y[i]) - 16.0*f(x[i-1],y[i-1]) + 5.0*f(x[i-2],y[i-2]))/12.0;
	}
