...
double integralA00(double a, double b, double *x, double *w){
	// integral from a to b of function (2 + Q(x-a)(x-b))(x-a)(x-b)
	double I = 0.0;		// Integral of function from a to b
	// Calculate integral I:
	double zeta;
	for(int i=0 ; i<=4 ; i++){
		zeta = (b-a)*x[i]/2.0 + (a+b)/2.0;
		I = I + w[i]*(2.0 + Q(zeta)*(zeta-a)*(zeta-b))*(zeta-a)*(zeta-b);
	}
	I = (b-a)*I/2.0;
	return I;
}
double integralA01(double a, double b, double *x, double *w){
	// integral from a to b of function (6x-4a-2b+Q(x-a)^2(x-b))(x-a)(x-b)
	double I = 0.0;		// Integral of function from a to b
	// Calculate integral I:
	double zeta;
	for(int i=0 ; i<=4 ; i++){
		zeta = (b-a)*x[i]/2.0 + (a+b)/2.0;
		I = I + w[i]*(6.0*zeta-4.0*a-2.0*b+Q(zeta)*pow((zeta-a),2.0)*(zeta-b))*(zeta-a)*(zeta-b);
	}
	I = (b-a)*I/2.0;
	return I;
}
double integralA10(double a, double b, double *x, double *w){
	// integral from a to b of function (2 + Q(x-a)(x-b))(x-a)^2(x-b)
	double I = 0.0;		// Integral of function from a to b
	// Calculate integral I:
	double zeta;
	for(int i=0 ; i<=4 ; i++){
		zeta = (b-a)*x[i]/2.0 + (a+b)/2.0;
		I = I + w[i]*(2.0 + Q(zeta)*(zeta-a)*(zeta-b))*pow((zeta-a),2.0)*(zeta-b);
	}
	I = (b-a)*I/2.0;
	return I;
}
double integralA11(double a, double b, double *x, double *w){
	// integral from a to b of function (6x-4a-2b+Q(x-a)^2(x-b))(x-a)^2(x-b)
	double I = 0.0;		// Integral of function from a to b
	// Calculate integral I:
	double zeta;
	for(int i=0 ; i<=4 ; i++){
		zeta = (b-a)*x[i]/2.0 + (a+b)/2.0;
		I = I + w[i]*(6.0*zeta-4.0*a-2.0*b+Q(zeta)*pow((zeta-a),2.0)*(zeta-b))*pow((zeta-a),2.0)*(zeta-b);
	}
	I = (b-a)*I/2.0;
	return I;
}
double integralq11(double a, double b, double *x, double *w){
...
double integralq21(double a, double b, double *x, double *w){
...
double integralf11(double a, double b, double *x, double *w){
...
double integralq31(double a, double b, double *x, double *w){
...
double integralf21(double a, double b, double *x, double *w){
...
int main(){
	...
	A[0][0] = integralA00(a,b,x,w);
	A[0][1] = integralA01(a,b,x,w);
	A[1][0] = integralA01(a,b,x,w);
	A[1][1] = integralA11(a,b,x,w);
	f[0] = -c[0]*integralq11(a,b,x,w)-c[1]*integralq21(a,b,x,w)+integralf11(a,b,x,w);
	f[1] = -c[0]*integralq21(a,b,x,w)-c[1]*integralq31(a,b,x,w)+integralf21(a,b,x,w);
