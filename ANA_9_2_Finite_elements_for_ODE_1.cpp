...
const int n = 7; 			// number of subintervals (elements)
...
void print_red(double M[n-1][n-1], int n){		// Displays matrix M
	for (int i=0 ; i<=n-2 ; i++){
		for (int j=0 ; j<=n-2 ; j++){
			std::cout << M[i][j] << " ";
		}
...
void print_vec_red(double b[n-1], int n){		// Displays vector b
	for (int i=0 ; i<=n-2 ; i++){
		std::cout << b[i] << "\n";
	}
}
...
int main(){
	...
	double x_0=0.0, x_n=2.0;		// Boundary values
	double y_a=0.0, y_b=3.5;		// Dirichlet conditions
	
	double x[n+1]={x_0,0.4,0.7,0.9,1.1,1.3,1.6,x_n};	// nodes of the intervals
...
//	}
	...
	b[1] = b[1] - K[1][0]*y_a;
	K[1][0] = 0.0;
	K[n][n] = 1.0;		K[n][n-1] = 0.0;
	b[n] = y_b;
	b[n-1] = b[n-1] - K[n-1][n]*y_b;
	K[n-1][n] = 0.0;
	...
	cout << "\nTHE SOLUTION IS:\nu = [";
	cout << u[0] << ", ";		// u[0] should be equal to y_a
	for (int i=1 ; i<=n-1 ; i++){
	 	cout << u[i] <<", ";}
	cout << u[n] <<"];";		// u[n] should be equal to y_b
