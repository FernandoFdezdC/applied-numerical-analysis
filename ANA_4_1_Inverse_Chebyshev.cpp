// GAUSS ELIMINATION METHOD APPLIED TO FIND x, x^2, ... AS A FUNTION OF
// CHEBYSHEV POLYNOMIALS T_0, T_1, ...
...
#include <math.h>
using namespace std;
const int n=11;			// dimension of the matrix
const int m=2*n;
...
int main(){
	...
	ifstream file;
    file.open("ANA_4_1_Chebyshev_matrix.txt", ios::in);
	for (int i=0 ; i<=n-1 ; i++){
		for (int j=0 ; j<=n-1 ; j++){
			file >> M[i][j];
		}
